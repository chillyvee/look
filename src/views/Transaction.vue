<template>
  <div>
    ...
    <b-card v-if="tx.logs" :title="`Logs (total: ${tx.logs.length})`" no-body>
      <h2>Logs</h2>
      <b-card-body
      ...
      >
        Event[{{ i }}]
        <b-card-body
          v-for="(jitem, j) in item.events"
          id="message"
          :key="j"
          class="message"
        >
          <object-field-component :tablefield="jitem" />
        </b-card-body>
      </b-card-body>
    </b-card>
  </div>
...
<script>
import { BCard, BTableSimple, BTr, BTd, BBadge, BCardBody } from 'bootstrap-vue'
