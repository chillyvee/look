<template>
  <div>
    <b-modal
      ...
      @ok="handleOk"
      @show="loadBalance"
      ><b-overlay :show="channels.length === 0" rounded="sm">
        <template #overlay>
...
export default {
  ...
  computed: {
    ...
    destinationOptions() {
      ...
      return query && String(this.token).startsWith('ibc/')
  ...
  methods: {
    ...
    loadBalance() {
      ...
      if (this.address) {
        this.$http
          ...
          .then(res => {
            if (res && res.length > 0) {
              ...
              this.feeDenom = this.balance.find(
                x => !x.denom.startsWith('ibc')
              ).denom
              this.balance
                ...
                .forEach(x => {
                  if (!this.IBCDenom[x.denom]) {
                    this.$http
                      .getIBCDenomTrace(x.denom, this.selectedChain)
                      .then(denom => {
        ...
        const channels = this.$store.state.chains.ibcChannels[
          ...
          this.$http.getIBCChannels(this.selectedChain, null).then(ret => {
            ...
            chans.forEach((x, i) => {
              this.$http
                ...
                .then(cs => {
                  chans[i].chain_id =
                    cs.identified_client_state.client_state.chain_id
                  this.$store.commit('setChannels', {
                    chain: this.selectedChain.chain_name,
                    channels: chans,
                  })
